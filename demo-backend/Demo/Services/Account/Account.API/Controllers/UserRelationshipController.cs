...
namespace Account.API.Controllers
{
    ...
    public class UserRelationshipController : ControllerBase
    {
        ...
        [HttpGet("FriendsPending")]
        [ProducesResponseType(typeof(ApiResponse<List<GetListFriendPendingVm>>), StatusCodes.Status200OK)]
        [ProducesDefaultResponseType]
        public async Task<IActionResult> GetListFriendsPending(
            [FromQuery] GetListFriendPending userRelationshipFilter)
        {
            var response = await _mediator.Send(userRelationshipFilter);
            return response.IsSuccess ? Ok(response) : StatusCode(int.Parse(response.ErrorCode), response);
        }
